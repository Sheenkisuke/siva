...
def _comparar_con_face_recognition(ruta_nueva, ruta_anterior):
    """
...
def _comparar_por_histograma(ruta_nueva, ruta_anterior):
    """
    ...
    try:
        ...
        nombre_archivo = os.path.basename(ruta_nueva).lower()
...
def comparar_rostros(ruta_foto_nueva, ruta_foto_anterior, umbral=0.85):
    """
    ...
    if not os.path.exists(ruta_foto_anterior):
    ...
    if not exito and FACE_REC_DISPONIBLE:
        ...
        exito, similitud = _comparar_con_face_recognition(ruta_foto_nueva, ruta_foto_anterior)
        if exito:
    ...
    if not exito:
        ...
        exito, similitud = _comparar_por_histograma(ruta_foto_nueva, ruta_foto_anterior)
